...
typedef struct {
  int             stateSwitch;                              // value 17 for Sleep mode; 66 for Work mode; 95 for Low Charge; any other value toggles after-power-down recovery
  int             programmCycles;                           // keeps track of time instead of Millis
  short           hour;
  short           minute;
  short           second;
  volatile int    controllsTracker;                         // inactivity timer. Can be changed by Button, Encoder and Tilt Detector
  volatile byte   encoderCounter;                           // encoder
  volatile bool   tiltSwitch;                               // tilt detector
} rtcManagementStruc;
rtcManagementStruc rtcMng;                                  // this struct is for bidirectional data transfer between SLEEP and WORK modes via RTC memory

typedef struct {
  volatile long   oldPosition;                              // encoder
  volatile bool   onPlane;                                  // Airplane Mode flag. 0 == off, 1 == on
  bool            onPlanePrevious;
  signed int      timeZone;
  short           day;
  short           weekDay;
  short           month;
...
short           x = 0;
short           y = 0;
const char*     serverName = "https://ap1.unwiredlabs.com/v2/process.php";
...
const String    ApiKey = "876dcc47a263d9592d6512f43f844417";                  // OpenWeather
